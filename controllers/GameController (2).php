...
class GameController extends Controller {
	...
	public function actionGameStart() {
		...
		$LobbyID = Yii::app() -> request -> getPost('LobbyID');
		$Lobby = new Lobby();
		// Если тип запроса AJAX:
		if (Yii::app() -> request -> isAjaxRequest) {
			// Если указанное лобби найдено:
	...
	public function actionGameFinish() {
		...
		$GameID = Yii::app() -> request -> getPost('GameID');
		$WinnerID = Yii::app() -> request -> getPost('WinnerID');
		$Game = new Game();
		// Если тип запроса AJAX:
		if (Yii::app() -> request -> isAjaxRequest) {
			// Если указанная игра найдена:
	...
	public function actionLobbyCreate() {
		...
		$SizeX = Yii::app() -> request -> getPost('SizeX');
		$SizeY = Yii::app() -> request -> getPost('SizeY');
		$ColorsNumber = Yii::app() -> request -> getPost('ColorsNumber');
		$PlayersNumber = Yii::app() -> request -> getPost('PlayersNumber');
